...
#begin
@log.trace("Started execution 'fb-cloud:docker:operation:create_container.rb' flintbit...") # execution Started
begin
	# Flintbit input parametes
	#Mandatory
	@connector_name = @input.get('connector_name') # docker connector name
	@action = 'create' # name of the operation: create
	@container_name = @input.get('container-name')	# name of container which you want create
	@image_name = @input.get('image-name')	# name of image which you want run into the container
	...
	if @connector_name.nil? || @connector_name.empty?
		raise 'Please provide "Docker connector name (@connector_name)" to create container'
	end

	#checking container name is nil or empty
	if @container_name.nil? || @container_name.empty?
		raise 'Please provide "container name (@container_name)"to create container'
	else
  ...
  if @image_name.nil? || @image_name.empty?
    raise 'Please provide "image name (@image_name)"to create container'
  else
...
@log.trace("Finished execution 'fb-cloud:docker:operation:create_container.rb' flintbit...")
#end
