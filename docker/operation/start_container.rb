...
#begin
@log.trace("Started execution 'fb-cloud:docker:operation:start_container.rb' flintbit...") # execution Started
begin
	# Flintbit input parametes
	#Mandatory
	@connector_name = @input.get('connector_name') # docker connector name
	@action = 'start' # name of the operation:start
	@container_id = @input.get('container-id')	# name of container which you want start
	...
	if @connector_name.nil? || @connector_name.empty?
		raise 'Please provide "Docker connector name (@connector_name)" to start container'
	end

	#checking container id is nil or empty
	if @container_id.nil? || @container_id.empty?
		raise 'Please provide "container id (@container_id)"to start container'
	else
...
@log.trace("Finished execution 'fb-cloud:docker:operation:start_container.rb' flintbit...")
#end
