#begin
@log.trace("Started execution 'fb-cloud:docker:operation:restart_container.rb' flintbit...") # execution Started
begin
	# Flintbit input parametes
	#Mandatory
	@connector_name = @input.get('connector_name') # docker connector name
	@action ='restart'# name of the operation:restart
	@container_id = @input.get('container-id')	# name of container which you want restart
	...
	if @connector_name.nil? || @connector_name.empty?
		raise 'Please provide "Docker connector name (@connector_name)" to restart container'
	end

	#checking container id is nil or empty
	if @container_id.nil? || @container_id.empty?
		raise 'Please provide "container id (@container_id)"to restart container'
	else
...
@log.trace("Finished execution 'fb-cloud:docker:operation:restart_container.rb' flintbit...")
#end
