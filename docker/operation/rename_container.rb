...
#begin
@log.trace("Started execution 'fb-cloud:docker:operation:rename_container.rb' flintbit...") # execution Started
begin
	# Flintbit input parametes
	#Mandatory
	@connector_name = @input.get('connector_name') # docker connector name
	@action ='rename-container' # name of the operation: rename-container
        @container_id = @input.get('container-id')	# id of the container which you want to rename
        @container_name = @input.get('container-name')	# new name of container which you want give to container
	...
	if @connector_name.nil? || @connector_name.empty?
		raise 'Please provide "Docker connector name (@connector_name)" to rename the container'
	end

  #checking container name is nil or empty
  if @container_name.nil? || @container_name.empty?
    raise 'Please provide "container name (@container_name)"to rename the container'
  else
    connector_call.set("container-name",@container_name)
  end

	#checking container id is nil or empty
  if @container_id.nil? || @container_id.empty?
    raise 'Please provide "container id (@container_id)"to rename the container'
  else
...
@log.trace("Finished execution 'fb-cloud:docker:operation:rename_container.rb' flintbit...")
#end
