# begin
@log.trace("Started executing 'fb-cloud:digitalocean:operation:reboot_instance.rb' flintbit...")
begin
    # Flintbit Input Parameters
    # Mandatory
    @connector_name = @input.get('connector_name') # Name of the DigitalOcean Connector
    @action = 'reboot' # Action (stop)
    ...
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "Digital Ocean connector name (connector_name)" to reboot Instance'
    end
    if @token.nil? || @token.empty?
        raise 'Please provide "Digital Ocean token (token)" to reboot instance'
    end
    ...
    actionType = response.get('action-type')       # Action type(REBOOT)
...
@log.trace("Finished executing 'fb-cloud:digitalocean:operation:reboot_instance.rb' flintbit...")
# end
