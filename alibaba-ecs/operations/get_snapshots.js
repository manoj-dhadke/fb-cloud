...
log.trace("Started executing fb-cloud:alibaba-ecs:operations:get_snapshots.js flintbit")
...
if (exit_code == 0) {
    for (index in connector_call_response.get('snapshots-list')) {
        log.trace(connector_call_response.get('snapshots-list')[index])
    }
    //log.trace("Snapshots list :: \n"+connector_call_response.get('zones-list'))
    output.set('user_message', connector_call_response.get('snapshots-list'))
    output.set('result', connector_call_response.get('snapshots-list'))
...
log.trace("Finished executing fb-cloud:alibaba-ecs:operations:get_snapshots.js flintbit")
