...
# begin
require 'json'
@log.trace("Started executing 'fb-cloud:azure:operation:describe_volume_by_name.rb' flintbit...")
begin
   ...
   @data_disk_name= @input.get('data-disk-name') #name of the data disk which you want to describe
   @group_name=@input.get('group-name') #name of the group in which your data disk is present

   @log.info("connector-name:#{@connector_name} | action :#{@action} | data-disk-id:#{@data_disk_name} | group-name:#{@group_name} ")
   ...
   #Checking that the data disk name is provided or not,if not then raise the exception with error message
   if @data_disk_name.nil? || @data_disk_name.empty?
       raise 'Please provide "MS Azure data disk name(@data_disk_name)" to describe volume'
   end

   # Checking that the resource group name is provided or not,if not then raise the exception with error message
   if @group_name.nil? || @group_name.empty?
       raise 'Please provide Azure  group name (@group_name) to describe volume'
   end
...
@log.trace("Finished executing 'fb-cloud:azure:operation:describe_volume_by_name.rb' flintbit")
# end
