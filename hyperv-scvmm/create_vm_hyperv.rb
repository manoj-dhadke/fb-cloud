...
require 'rubygems'
#begin
@log.trace("Started executing 'fb-cloud:hyperv:operation:create_vm_hyperv.rb' flintbit...")
begin
    #Flintbit Input Parameters
    #Mandatory  
    @connector_name= @input.get("connector_name")                             #Name of the Connector
    @action="create-vm-hyperv"
    @vm_name= @input.get("vm-name")
    @ram_memory= @input.get("ram-memory-in-bytes")
    @path= @input.get("vm-path")
    @cpu= @input.get("cpu")   
    @vhd_memory= @input.get("vhd-memory-in-bytes")
    ...
    connector_call = @call.connector(@connector_name)
	            .set("vm-name",@vm_name)
	            .set("ram-memory-in-bytes",@ram_memory)
	            .set("vm-path",@path)
	            .set("cpu",@cpu)
	            .set("vhd-memory-in-bytes",@vhd_memory)
                    .set("target",@target)
...
@log.trace("Finished executing 'fb-cloud:hyperv:operation:create_vm_hyperv.rb' flintbit...")
#end
