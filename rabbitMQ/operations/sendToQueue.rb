	...
	@log.trace("Started execution of 'fb-cloud:rabbitMQ:operations:sendToQueue.rb' flintbit..")
begin
	...
	input_to_queue = @input.get('data_to_queue')
	...
	# Optional
		...
		connector_call = @call.connector(connector_name)
		                          ...
		                          .set('input-to-queue',input_to_queue)
		                          .set('action',action)
		...
		#checking queue name is nil or empty
		    if !queue_name.nil? && !queue_name.empty?
				...
				@log.error("Please provide queue name to send")
			end
	...
	@log.trace("Finished execution of 'fb-cloud:rabbitMQ:operations:sendToQueue.rb' flintbit..")