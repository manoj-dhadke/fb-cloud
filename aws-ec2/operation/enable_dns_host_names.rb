# begin
@log.trace("Started executing 'fb-cloud:aws-ec2:operation:enable_dns_host_names.rb' flintbit... &")
begin
    ...
    is_enable_dns_host_names = @input.get('is-enable-dns-host-names') # provide true to enable dns host names other-wise false
    ...
    @log.info("Flintbit input parameters are,connector_name:#{connector_name} | action : #{action} | vpc-id :#{vpc_id}| is-enable-dns-host-names:-#{is_enable_dns_host_names}")
    ...
    connector_call = @call.connector(connector_name)
                          .set('action', action)
                          .set('vpc-id', vpc_id)
                          .set('is-enable-dns-host-names', is_enable_dns_host_names)
...
@log.trace("Finished executing 'fb-cloud:aws-ec2:operation:enable_dns_host_names.rb' flintbit")
# end
