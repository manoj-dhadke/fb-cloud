...
*Summary: To delete a Load Balancer
*Description: To Delete a Load balancer using aws-ec2 Connector
 */

log.trace("Started executing the flintbit 'fb-cloud:aws-ec2:operation:workflow:delete_load_balancer.js'");
...
if(input_scope.hasOwnProperty("cloud_connection")){
    ...
    //Load Balancer Name - depends on type of load balancer
    if(input_scope.hasOwnProperty("load_balancer_name")){
        ...
        if(load_balancer_name!=null || load_balancer_name!=""){
            ...
            log.info("Load Balancer Name is null or empty string.")
        }
    }
    ...
    //Load Balancer Type & Action - mandatory
    if(input_scope.hasOwnProperty("load_balancer_type")){
        ...
        if(load_balancer_type!=null || load_balancer_type!=""){ 
            ...
            //Action
            action = "delete-"+load_balancer_type+"-load-balancer";
    ...
    //Load Balancer ARN - depends on type of load balancer
    if(load_balancer_type=="application"){
        if(input_scope.hasOwnProperty("load_balancer_arn")){
            ...
            if(load_balancer_arn!=null || load_balancer_arn!=""){
                ...
                log.info("load_balancer_arn is null or empty string.")
            }
        }
        else{  //load_balancer_arn key not present in input JSON 
            log.info("Input does not contain the key 'load_balancer_arn'")
        }
    }
    ...
    if(response_exitcode==0){
        log.info("Successfully deleted the Load Balancer.");
        output.set("message",response_message)
            .set("exit-code",response_exitcode)
            .set("user_message","Successfully deleted the Load Balancer.");
        log.trace("Finished executing 'fb-cloud:aws-ec2:operation:workflow:delete_load_balancer.js' successfully");
    }
    else{
        log.error("Failure in execution, message:"+response_message+" | exitcode:"+response_exitcode);
        output.set("error",response_message).set("exit-code",-1);
        log.trace("finished executing 'fb-cloud:aws-ec2:operation:workflow:delete_load_balancer.js' with errors")
    }
}
