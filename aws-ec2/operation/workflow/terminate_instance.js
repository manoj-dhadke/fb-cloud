 ...
 * Summary: To terminate one or more instance on AWS
 * Description: To terminate one or more instance on AWS using aws-ec2 connector
 */

log.trace("Started executing the flintbit 'fb-cloud:aws-ec2:operation:workflow:terminate_instance.js'");
...
if(input_scope.hasOwnProperty("cloud_connection")){
    ...
    if(response_exitcode==0){
        ...
        log.trace("Finished executing the flintbit 'fb-cloud:aws-ec2:operation:workflow:terminate_instance.js' successfully");
    }
    else{
        log.error("Failure in execution, message:"+response_message+" | exitcode:"+response_exitcode);
        output.set("error",response_message).set("exit-code",-1);
        log.trace("finished executing 'fb-cloud:aws-ec2:operation:workflow:terminate_instance.js' with errors")
    }
}
