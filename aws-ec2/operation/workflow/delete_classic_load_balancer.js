...
log.trace("Started executing the flintbit 'fb-cloud:aws-ec2:operation:workflow:delete_classic_load_balancer.js'");
...
if(input_scope.hasOwnProperty("cloud_connection")){
    ...
    if(response_exitcode==0){
        log.info("Successfully deleted the Classic Load Balancer.");
        output.set("message",response_message)
            .set("exit-code",response_exitcode)
            .set("user_message","Successfully deleted the Classic Load Balancer.");
        log.trace("Finished executing 'fb-cloud:aws-ec2:operation:workflow:delete_classic_load_balancer.js' successfully");
    }
    else{
        log.error("Failure in execution, message:"+response_message+" | exitcode:"+response_exitcode);
        output.set("error",response_message).set("exit-code",-1);
        log.trace("finished executing 'fb-cloud:aws-ec2:operation:workflow:delete_classic_load_balancer.js' with errors")
    }
}
