#begin
@log.trace("Started executing 'flint-cloud:aws-ec2:operation:create_amazon_instance.rb' flintbit...")
begin
...
@log.info("Flintbit input parameters are, action : #{action} | image_id : #{image_id} | availability_zone : #{availability_zone} |
instance_type : #{instance_type} | key_name : #{key_name} | min_instance : #{min_instance} | max_instance : #{max_instance} | region : #{region} | subnet_id : #{subnet_id}") 
...
connector_call = @call.connector(connector_name).set("action",action).set("image-id",image_id).set("instance-type",instance_type)
.set("min-instance",min_instance).set("max-instance",max_instance).set("access-key",@access_key).set("security-key",@secret_key)
...
if response_exitcode == 0
	...
	@output.set("exit-code",0).setraw("instances-info",instance_info.to_s)
else
	...
	@log.error(e.message)
	@output.set("exit-code",0).set("message",e.message)
end
	@log.trace("Finished executing 'flint-cloud:aws-ec2:operation:create_amazon_instance.rb' flintbit")
#end
