 ...
 */
log.trace("Started executing 'fb-cloud:aws-ec2:operation:stop_instance.groovy' flintbit...")
try{
    ...
    if (connector_name == null || connector_name == ""){
        throw new Exception('Please provide "Amazon EC2 connector name (connector_name)" to stop Instance')
    }

    if (instance_id == null || instance_id == ""){
        throw new Exception('Please provide "Amazon instance ID (instance_id)" to stop Instance')
    }
    else{
    ...
    if (response_exitcode == 0){
        ...
        output.set('exit-code', 0).set('stopped-instances', instances_set).set('user_message',user_message)
    }
    else{
        ...
        if (response != ""){
        output.set('message', response_message).set('exit-code', 1).set('error-details',response.toString()).set('user_message',user_message)
        }
        else{
...
log.trace("Finished executing 'fb-cloud:aws-ec2:operation:stop_instance.groovy' flintbit")
// end
