# begin
@log.trace("Started executing 'fb-cloud:aws-ec2:operation:describe_security_group_by_name.rb' flintbit...")
begin
# Flintbit Input Parameters
# Mandatory
connector_name = @input.get('connector_name')	# Name of the Amazon EC2 Connector
action = 'describe-security-group' # Specifies the name of the operation:describe-security-group
security_group_name = @input.get('security-group-name') # Contains one or more security group name corresponding to the region that you want to describe
# Optional
...
@log.info("Flintbit input parameters are, action : #{action} | security-group-name: #{security_group_name} | region:#{region}")
...
if response_exitcode == 0
    ...
    @output.set('message', response_message).set('exit-code', 1).set('error-details',response.to_s)
...
@log.trace("Finished executing 'fb-cloud:aws-ec2:operation:describe_security_group_by_name.rb' flintbit")
# end
