...
try{
    ...
    action = 'take-snapshot-of-volume' // Specifies the name of the operation:take-snapshot-of-volume
    volume_id = input.get('volume-id')// Specifies the id of the volume
    description = input.get('description')// Specifies the description to put on the snapshot of the volume
    // Optional
    ...
    log.info("Flintbit input parameters are, action : ${action} | volume_id : ${volume_id} | snapshot description : ${description} | region : ${region} ")

    if (connector_name ==null || connector_name ==""){
        throw new Exception('Please provide "Amazon EC2 connector name (connector_name)" to take snapshot of volume')
    }

    if (volume_id ==null || volume_id ==""){
        throw new Exception('Please provide "Amazon EC2 volume ID (volume_id)" to take snapshot of a volume')
    }

    if (description ==null || description ==""){
        throw new Exception('Please provide "Amazon EC2 snapshot description" to take snapshot of a volume')
    }
