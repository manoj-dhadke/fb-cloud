 ...
 */
log.trace("Started executing 'fb-cloud:aws-ec2:operation:reboot_instance.groovy' flintbit...")
try{
    ...
    if (connector_name ==null || connector_name ==""){
        throw new Exception('Please provide "Amazon EC2 connector name (connector_name)" to reboot Instance')
    }

    if (instance_id ==null || instance_id ==""){
        throw new Exception('Please provide "Amazon instance ID (instance_id)" to reboot Instance')
    }
    else{
    ...
    if (response_exitcode == 0){
        ...
        output.set('exit-code', 0).setraw('rebooted-instances', instances_set.toString())
    }
    else{
...
log.trace("Finished executing 'fb-cloud:aws-ec2:operation:reboot_instance.groovy' flintbit")
// end
