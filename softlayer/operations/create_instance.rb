...
begin
    ...
    # Name of the Softlayer Connector
    @action = 'create' # Contains the name of the operation: create
    ...
    @log.info("Flintbit input parameters are, connector name :: #{@connector_name} | action :: #{@action}| hostname :: #{@hostname}|
    domainname :: #{@domainname}|cpu :: #{@cpu} |maxmemory :: #{@maxmemory}|datacenter :: #{@datacenter}| operating_system :: #{@operating_system}|
    username :: #{@username}|apikey :: #{@apikey} |timeout :: #{@request_timeout}")

    if @hostname.nil? || @hostname.empty?
        raise 'Please provide "softalyer vm hostname (host-name)" to launch instance'
    end
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "softalyer connector name (connector_name)" to launch instance'
    end
    if @domainname.nil? || @domainname.empty?
        raise 'Please provide "softlayer domain name (domain-name) " to launch instance'
    end
    if @datacenter.nil? || @datacenter.empty?
        raise 'Please provide "softalyer data center (datacenter)" to launch instance'
    end
    if @operating_system.nil? | @operating_system.empty?
        raise 'Please provide "softlayer operating system (operating-system)" to launch instance'
    end
