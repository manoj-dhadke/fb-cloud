...
// begin
log.trace("Started execution 'fb-cloud:vmware-vcd:operation:get_virtual_machine_details.groovy' flintbit...") // execution Started
try{
    ...
    if(connector_name==null || connector_name==""){
        throw new Exception('Please provide "VMWare connector name (connector_name)"  to get details of virtual machines')
    }
  
    // checking host name is nil or empty
    if(hostname==null || hostname==""){
        throw new Exception('Please provide "vCloud director server hostname(host-name)"  to get details of virtual machines')
    }
	
    // checking organization name is nil or empty
    if(organization_name==null || organization_name==""){
        throw new Exception('Please provide "organization name (organization-name)" to get details of virtual machines')
    }
  
    // checking organization admin username is nil or empty
    if(organization_admin_username==null || organization_admin_username==""){
        throw new Exception('Please provide "Organzation admin username (Organzation-admin-username)"  to get details of virtual machines')
    }
   
    // checking organization admin password is nil or empty
    if(organization_admin_password==null || organization_admin_password==""){
        throw new Exception('Please provide "Organzation admin password (Organzation-admin-password)"  to get details of virtual machines')
    }

    // checking protocol is nil or empty
    if(protocol==null || protocol==""){
        throw new Exception('Please provide "protocol (protocol)"  to get details of virtual machines')
    }

    // checking vm-name is nil or empty
    if(vm_name==null || vm_name==""){
        throw new Exception('Please provide "virtual machine name (vm-name)" to get details of virtual machines')
    }
    ...
    if(response_exitcode == 0){
        ...
        output.set('vm-details', response.toString()).set('exit-code', 0).set('message',response_message)
    }
    else{
...
log.trace("Finished execution 'fb-cloud:vmware-vcd:operation:get_virtual_machine_details.groovy' flintbit...")
// end