# begin
@log.trace("Started execution 'fb-cloud:vmware-vcd:operation:delete_virtual_machine.rb' flintbit...") # execution Started
begin
    ...
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "VMWare connector name (connector_name)"  to delete  virtual machine'
    end
  
    # checking host name is nil or empty
    if @hostname.nil? || @hostname.empty?
        raise 'Please provide "vCloud director server hostname(host-name)" to delete virtual machine'
    end
	
     # checking organization name is nil or empty
    if @organization_name.nil? || @organization_name.empty?
        raise 'Please provide "organization name (organization-name)" to delete virtual machine'
    end
  
    # checking organization admin username is nil or empty
    if @organization_admin_username.nil? || @organization_admin_username.empty?
        raise 'Please provide "Organzation admin username (Organzation-admin-username)"  to delete virtual machine'
    end
   
    # checking organization admin password is nil or empty
    if @organization_admin_password.nil? || @organization_admin_password.empty?
        raise 'Please provide "Organzation admin password (Organzation-admin-password)"  to delete virtual machine'
    end
    ...
    if @vm_name.nil? || @vm_name.empty?
        raise 'Please provide "virtual machine name (vm-name)"to delete virtual machine'
    end
...
@log.trace("Finished execution 'fb-cloud:vmware-vcd:operation:delete_virtual_machine.rb' flintbit...")
# end
