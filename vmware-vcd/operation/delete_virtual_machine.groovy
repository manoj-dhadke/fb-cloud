 ...
 */
log.trace("Started execution 'fb-cloud:vmware-vcd:operation:delete_virtual_machine.groovy' flintbit...") // execution Started
try{

    // Flintbit Input Parameters
    // Mandatory
    connector_name = input.get('connector_name') // vmware-vcd connector name
    action ='delete-vm' // name of action:delete-vm
    ...
    vm_name = input.get('vm_name') //vm-name to delete
    ...
    if(connector_name==null || connector_name==""){
        throw new Exception('Please provide "VMWare connector name (connector_name)"  to delete  virtual machine')
    }
  
    // checking host name is nil or empty
    if(hostname==null || hostname==""){
        throw new Exception('Please provide "vCloud director server hostname(host-name)" to delete virtual machine')
    }
	
    // checking organization name is nil or empty
    if(organization_name==null || organization_name==""){
        throw new Exception('Please provide "organization name (organization-name)" to delete virtual machine')
    }
  
    // checking organization admin username is nil or empty
    if(organization_admin_username==null || organization_admin_username==""){
        throw new Exception('Please provide "Organzation admin username (Organzation-admin-username)"  to delete virtual machine')
    }
   
    // checking organization admin password is nil or empty
    if(organization_admin_password==null || organization_admin_password==""){
        throw new Exception('Please provide "Organzation admin password (Organzation-admin-password)"  to delete virtual machine')
    }

    // checking protocol is nil or empty
    if(protocol==null || protocol==""){
        throw new Exception('Please provide "protocol (protocol)"  to start of virtual machine')
    }

    // checking vm-name is nil or empty
    if(vm_name==null || vm_name==""){
        throw new Exception('Please provide "virtual machine name (vm-name)"to delete virtual machine')
    }
...
log.trace("Finished execution 'fb-cloud:vmware-vcd:operation:delete_virtual_machine.groovy' flintbit...")
// end