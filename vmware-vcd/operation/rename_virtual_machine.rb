...
# begin
@log.trace("Started execution 'fb-cloud:vmware-vcd:operation:rename_virtual_machine.rb' flintbit...") # execution Started
begin

    # Flintbit Input Parameters
    # Mandatory
    @connector_name = @input.get('connector_name') # vmware-vcd connector name
    @action ='rename-vm' # name of action:rename-vm
    ...
    @vm_name = @input.get('vm_name') #vm_name to rename virtual machine
    @new_vm_name = @input.get('new_vm_name') #new_vm_name to rename virtual machine
    ...
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "VMWare connector name (connector_name)"  to rename of virtual machine'
    end
  
    # checking host name is nil or empty
    if @hostname.nil? || @hostname.empty?
        raise 'Please provide "vCloud director server hostname(host-name)" to rename of virtual machine'
    end
	
     # checking organization name is nil or empty
    if @organization_name.nil? || @organization_name.empty?
        raise 'Please provide "organization name (organization-name)" to rename of virtual machine'
    end
  
    # checking organization admin username is nil or empty
    if @organization_admin_username.nil? || @organization_admin_username.empty?
        raise 'Please provide "Organzation admin username (Organzation-admin-username)"  to rename of virtual machine'
    end
   
    # checking organization admin password is nil or empty
    if @organization_admin_password.nil? || @organization_admin_password.empty?
        raise 'Please provide "Organzation admin password (Organzation-admin-password)"  to rename of virtual machine'
    end

    # checking protocol is nil or empty
    if @protocol.nil? || @protocol.empty?
        raise 'Please provide "protocol (protocol)"  to  rename of virtual machine'
    end

    # checking vm-name is nil or empty
    if @vm_name.nil? || @vm_name.empty?
        raise 'Please provide "virtual machine name (vm_name)"to rename of virtual machine'
    end

     # checking new-vm-name is nil or empty
    if @new_vm_name.nil? || @new_vm_name.empty?
        raise 'Please provide "New virtual machine name (new_vm_name)"to rename of virtual machine'
    end
...
@log.trace("Finished execution 'fb-cloud:vmware-vcd:operation:rename_virtual_machine.rb' flintbit...")
# end
