...
# begin
@log.trace("Started execution 'fb-cloud:vmware-vcd:operation:get_virtual_machine_details.rb' flintbit...") # execution Started
begin
    ...
    if @connector_name.nil? || @connector_name.empty?
        raise 'Please provide "VMWare connector name (connector_name)"  to get details of virtual machines'
    end
  
    # checking host name is nil or empty
    if @hostname.nil? || @hostname.empty?
        raise 'Please provide "vCloud director server hostname(host-name)"  to get details of virtual machines'
    end
	
     # checking organization name is nil or empty
    if @organization_name.nil? || @organization_name.empty?
        raise 'Please provide "organization name (organization-name)" to get details of virtual machines'
    end
  
    # checking organization admin username is nil or empty
    if @organization_admin_username.nil? || @organization_admin_username.empty?
        raise 'Please provide "Organzation admin username (Organzation-admin-username)"  to get details of virtual machines'
    end
   
    # checking organization admin password is nil or empty
    if @organization_admin_password.nil? || @organization_admin_password.empty?
        raise 'Please provide "Organzation admin password (Organzation-admin-password)"  to get details of virtual machines'
    end

    # checking protocol is nil or empty
    if @protocol.nil? || @protocol.empty?
        raise 'Please provide "protocol (protocol)"  to get details of virtual machines'
    end

    # checking vm-name is nil or empty
    if @vm_name.nil? || @vm_name.empty?
        raise 'Please provide "virtual machine name (vm-name)" to get details of virtual machines'
    end
...
@log.trace("Finished execution 'fb-cloud:vmware-vcd:operation:get_virtual_machine_details.rb' flintbit...")
# end
