...
Description - To Create a Bucket on AWS S3
*/

log.trace("Started executing 'fb-cloud:aws-s3:operation:create_bucket.groovy' flintbit...")
try{
	// Flintbit Input Parameters
	// Mandatory Input Parameters
	connector_name = input.get("connector_name") // Name of the aws-s3 connector
	action = 'create-bucket'                    //input.get("action")
	bucket_name = input.get("bucket-name")  //name of the bucket which you want to create
	...
	if(bucket_name==null || bucket_name==""){
		throw new Exception('Please provide "name of bucket (bucket-name)" to create bucket on aws-s3')
    }
    ...
    //initializing the connector call with the parameter
	...
	// Checking that timeout proivded or not
	...
	log.error('output in exception')
}

log.trace("Finished executing 'fb-cloud:aws-s3:operation:create_bucket.groovy' flintbit")