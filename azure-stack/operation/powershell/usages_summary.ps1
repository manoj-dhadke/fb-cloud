...
function Export-AzsUsage {
    ...
    if (Test-Path -Path $JsonFile -ErrorAction SilentlyContinue)
    {
        if ($Force)
        {
            ...
            Write-Error "'$JsonFile' already exists use -Force to overwrite"
            return
        }
    }
    ...
    $usageSummary | ConvertTo-Json | Add-Content -Path $JsonFile 
    if ($PSBoundParameters.ContainsKey('Debug'))
    {
        $result | ConvertTo-Json | Add-Content -Path "$JsonFile.parse"
    }
...
Export-AzsUsage -StartTime 7/30/2018 -EndTime $endTime -Granularity Monthly -Force -TenantUsage