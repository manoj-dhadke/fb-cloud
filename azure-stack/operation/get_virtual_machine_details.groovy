...
log.trace("Started executing 'fb-cloud:azure-stack:operation:get_virtual_machine_details.groovy' flintbit...")
try{
    ...
    if (import_exitcode == 0){
	     ...
	     login_command="cd C:\\AzureStack-Tools-master; .\\azureimport_script2.ps1 ${subscription_name} ${tenant_username} ${tenant_password} ${aadtenant_name};.\\singlevmdetails.ps1 ${virtual_machine_name} ${resource_group_name} 2>&1|convertto-json"
		...
		if (login_azure_stack_exitcode == 0){
			 ...
			 output.set('exit-code', 0).set('message', login_azure_stack.message()).set('vm-details',result.toString())
...
log.trace("Finished executing 'fb-cloud:azure-stack:operation:get_virtual_machine_details.groovy' flintbit...")