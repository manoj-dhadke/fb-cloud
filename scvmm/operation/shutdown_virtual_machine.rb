...
require 'rubygems'
#begin
@log.trace("Started executing 'fb-cloud:scvmm:operation:shutdown_virtual_machine.rb' flintbit...")
begin
    ...
    @command = "$VM = Get-SCVirtualMachine -Id  #{@vmidentifier} -VMMServer #{@target};Stop-SCVirtualMachine -VM $VM -Shutdown| convertto-json"                    #Command to run
    ...
    if @vmidentifier == nil || @vmidentifier == ""
            @log.error("Please provide vm id to perform stop operation")
...
@log.trace("Finished executing 'fb-cloud:scvmm:operation:shutdown_virtual_machine.rb' flintbit...")
#end
