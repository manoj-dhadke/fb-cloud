...
require 'rubygems'
#begin
@log.trace("Started executing 'fb-cloud:scvmm:operation:pause_virtual_machine.rb' flintbit...")
begin
    ...
    @command = "$VM = Get-SCVirtualMachine -Id  #{@vmidentifier} -VMMServer #{@target};Suspend-SCVirtualMachine -VM $VM | convertto-json"                    #Command to run
    ...
    if @vmidentifier == nil || @vmidentifier == ""
            @log.error("Please provide vm id to perform pause operation")
...
@log.trace("Finished executing 'fb-cloud:scvmm:operation:pause_virtual_machine.rb' flintbit...")
#end
