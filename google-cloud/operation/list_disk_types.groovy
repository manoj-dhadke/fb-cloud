// begin
log.trace("Started execution 'fb-cloud:google-cloud:operation:list_disk_types.groovy' flintbit...") // execution Started
try{
    // Flintbit input parametes
    // Mandatory
    connector_name = input.get('connector_name') // google-cloud connector name
    action = "list-disk-types" // name of the operation:list-disk-types
    ...
    if (connector_name == null || connector_name == ""){
       throw new Exception ( 'Please provide "google-cloud connector name (connector_name)" to list disk types')
    }

    //checking project-id is nil or empty
    if( project_id == null || project_id == ""){
       throw new Exception ( 'Please provide "Please provide project id (project-id)" to list disk types')
    }
    else{
    ...
    if( zone_name == null || zone_name == ""){
       throw new Exception ( 'Please provide "Please provide zone name(zone-name)" to list disk types')
    }
    else{
  ...
  //checking service-account-credentials is nil or empty
    if( service_account_credenetials == null || service_account_credenetials == ""){
       throw new Exception ( 'Please provide "Please provide service account credentials (service-account-credentials)" to list disk types')
    }
    else{
    ...
    //list of the regions
    disk_type_list=response.get('disk-type-list')

    if (response_exitcode == 0){
        log.info("Success in executing ${connector_name} Connector, where exitcode :: ${response_exitcode} | message :: ${response_message}")      
            output.set('exit-code', 0).set('message', 'success').set('disk-type-list',disk_type_list)       
    }
    else{
...
log.trace("Finished execution 'fb-cloud:google-cloud:operation:list_disk_types.groovy' flintbit...")
// end
