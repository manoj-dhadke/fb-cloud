 ...
 */
log.trace("Started execution 'fb-cloud:google-cloud:operation:list_regions.groovy' flintbit...") // execution Started
try{
    // Flintbit input parametes
    // Mandatory
    connector_name = input.get('connector_name') // google-cloud connector name
    action = "list-regions" // name of the operation: list-regions
    ...
    if (connector_name == null || connector_name == ""){
       throw new Exception ( 'Please provide "google-cloud connector name (connector_name)" to list regions')
    }
    ...
    if( project_id == null || project_id == ""){
       throw new Exception ( 'Please provide "Please provide project id (project-id)" to list regions')
    }
    else{
  ...
  //checking service-account-credentials is nil or empty
    if( service_account_credenetials == null || service_account_credenetials == ""){
       throw new Exception ( 'Please provide "Please provide service account credentials (service-account-credentials)" to list regions')
    }
    else{
    ...
    //list of the regions
    regions_list=response.get('regions-list')

    if (response_exitcode == 0){
        log.info("Success in executing ${connector_name} Connector, where exitcode :: ${response_exitcode} | message :: ${response_message}")
        output.set('exit-code', 0).set('message', 'success').set('region-list',regions_list)
    }
    else{
...
log.trace("Finished execution 'fb-cloud:google-cloud:operation:list_regions.groovy' flintbit...")
// end
