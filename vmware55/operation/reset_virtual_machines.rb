# begin
@log.trace("Started execution 'fb-cloud:vmware55:operation:reset_virtual_machines.rb' flintbit...") # execution Started
begin
    # Flintbit input parametes
    # Mandatory
    @connector_name = @input.get('connector_name') # vmware connector name
    @action = @input.get('action') # name of the operation:reset-vm
    @username = @input.get('username') # username of vmware connector
    @password = @input.get('password') # password of vmware connector
    @vmname = @input.get('vm-name')	# name of virtual machine which you want to reset
...
@log.trace("Finished execution 'fb-cloud:vmware55:operation:reset_virtual_machines.rb' flintbit...")
# end
